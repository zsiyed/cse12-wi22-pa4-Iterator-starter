...
public class MyLinkedListPublicTester {
    ...
    public void testConstructor() {
        ...
        assertEquals("The data of initial iterator's left", null, listLen1Iter.left.data);
        assertEquals("The data of initial iterator's right", "Christine", listLen1Iter.right.data);
    }

    @Test
    public void testHasNext() {
        assertTrue("call hasNext when there is a next node", listLen2Iter.hasNext());
        ...
        listLen2Iter.canRemoveOrSet = true;
        listLen2Iter.forward = true;
        assertFalse("call hasNext when there isn't a next node", listLen2Iter.hasNext());
    }

    @Test
    public void testNextOneCall() {
        assertEquals("The element returned from calling next", "Christine", listLen1Iter.next());
    ...
    public void testNextTwoCalls() {
        assertEquals("The element returned from calling next", "Paul", listLen2Iter.next());
        assertEquals("Index of iterator after 1 next()", 1, listLen2Iter.idx);
        assertEquals("The element returned from calling next", "Cao", listLen2Iter.next());
    ...
    public void testHasPrev() {
        assertFalse("call when there isn't a previous node", listLen2Iter.hasPrevious());
        // Manually doing listLen2Iter.next();
        listLen2Iter.left = listLen2.head.getNext();
        listLen2Iter.right = listLen2.head.getNext().getNext();
        listLen2Iter.idx = 1;
        listLen2Iter.canRemoveOrSet = true;
        listLen2Iter.forward = true;
        assertTrue("call when there is a previous node", listLen2Iter.hasPrevious());
        // Manually doing listLen2Iter.next();
        listLen2Iter.left = listLen2.head.getNext().getNext();
        listLen2Iter.right = listLen2.head.getNext().getNext().getNext();
        listLen2Iter.idx = 2;
    ...
    public void testPreviousOneCall() {
        ...
        assertEquals("The element returned from calling previous", "Christine", listLen1Iter.previous());
        assertEquals("Index of iterator after 0 previous()", 0, listLen1Iter.idx);
        assertTrue("Able to remove node", listLen1Iter.canRemoveOrSet);
    ...
    public void testPreviousNoPrevious() {
        ...
        listLen1Iter.forward = true;
        assertEquals("The element returned from calling previous", "Christine", listLen1Iter.previous());
        assertEquals("Index of iterator after 2 previous()", 0, listLen1Iter.idx);
        assertTrue("Able to remove node", listLen1Iter.canRemoveOrSet);
        assertFalse("Direction is not forward", listLen1Iter.forward);
        try {
    ...
    public void testPrevIndex() {
        ...
        listLen2Iter.forward = true;
        listLen2Iter.canRemoveOrSet = true;
        assertEquals("Index after 2 next, no previous", 1, listLen2Iter.previousIndex());
        listLen2Iter.left = listLen2.head.getNext();
        listLen2Iter.right = listLen2.head.getNext().getNext();
        listLen2Iter.idx = 1;
        listLen2Iter.forward = true;
        listLen2Iter.canRemoveOrSet = true;
        assertEquals("Index after 2 next, 1 previous", 0, listLen2Iter.previousIndex());
    }

    @Test
    public void testPrevIndexFirstNode() {
        assertEquals("Previous index at first node", -1, listLen2Iter.previousIndex());
    }
    ...
    public void testSetBackward() {
        ...
        listLen2Iter.set("Did it change?");
        assertEquals("Valid set when backward", "Did it change?", listLen2Iter.right.getElement());
        assertEquals("Valid set when backward, shouldn't change", "Paul", listLen2Iter.left.getElement());
    }
    ...
    public void testRemoveForward() {
        ...
        listLen2Iter.remove();
        assertEquals("Valid remove when forward", null, listLen2Iter.left.getElement());
        assertEquals("Valid remove when forward, shouldn't change", "Cao", listLen2Iter.right.getElement());
        assertEquals("Index should decrement after removal", 0, listLen2Iter.idx);
        assertFalse("Prevent another remove or set", listLen2Iter.canRemoveOrSet);
    }
    ...
    public void testRemoveBackward() {
        ...
        listLen2Iter.remove();
        assertEquals("Valid remove when forward", null, listLen2Iter.right.getElement());
        assertEquals("Valid remove when backward, shouldn't change", "Paul", listLen2Iter.left.getElement());
        assertEquals("Index shouldn't decrement after removal", 1, listLen2Iter.idx);
        assertFalse("Prevent another remove or set", listLen2Iter.canRemoveOrSet);
    }
    ...
    public void testAddValidIndex() {
        ...
        assertEquals("Valid add left", "CSE 12", listLen2Iter.left.getElement());
        assertEquals("Valid add right", "Cao", listLen2Iter.right.getElement());
        assertEquals("Index change after add", 2, listLen2Iter.idx);
        assertFalse("Cannot remove immediately after add", listLen2Iter.canRemoveOrSet);
    }
    ...
    public void testAddFirstIndex() {
        listLen2Iter.add("CSE 12");
        assertEquals("Valid add left", "CSE 12", listLen2Iter.left.getElement());
        assertEquals("Valid add right", "Paul", listLen2Iter.right.getElement());
